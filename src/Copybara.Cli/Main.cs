...
public class Main
{
    ...
    /// <summary>
    /// Build metadata about this binary. Upstream reads a <c>/build-data.properties</c> resource
    /// generated by Bazel; here the equivalent information comes from the assembly attributes the
    /// .NET SDK stamps from <c>Version</c> (plus the commit SHA when Source Link is enabled).
    /// </summary>
    private static ImmutableDictionary<string, string> GetBuildInfo()
    {
        ...
        var buildData = ImmutableDictionary.CreateBuilder<string, string>();
        buildData[BuildLabel] = informationalVersion;
        buildData["Build target"] = assembly.GetName().Name ?? "copybara";
        buildData["Build runtime"] = RuntimeInformation.FrameworkDescription;
        return buildData.ToImmutable();
    }
